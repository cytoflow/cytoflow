...
from cytoflowgui.workflow.views.parallel_coords import (ParallelCoordinatesWorkflowView, 
...
class ParallelCoordinatesParamsHandler(Controller):
    view_params_view = \
        View(Item('alpha',
...
class ParallelCoordinatesHandler(ViewHandler):
...
class ParallelCoordinatesPlugin(Plugin, PluginHelpMixin):
    ...
    def get_handler(self, model, context):
        if isinstance(model, ParallelCoordinatesWorkflowView):
            return ParallelCoordinatesHandler(model = model, context = context)
        elif isinstance(model, ParallelCoordinatesPlotParams):
            return ParallelCoordinatesParamsHandler(model = model, context = context)
